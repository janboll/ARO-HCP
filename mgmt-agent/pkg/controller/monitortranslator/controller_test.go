...
func newTestMonitorController(t *testing.T, dynamicClient *dynamicfake.FakeDynamicClient, nsPrefix string, sources ...*unstructured.Unstructured) *MonitorTranslatorController {
	t.Helper()
...
func TestSyncHandler(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "source not in matching namespace — skip",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c := newTestMonitorController(t, fakeDynClient, tt.nsPrefix, sources...)
